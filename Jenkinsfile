      ...
      steps{
        script {
          docker.withRegistry( 'aws ecr get-login-password --region ap-south-1 | docker login --username AWS --password-stdin 179625949269.dkr.ecr.ap-south-1.amazonaws.com') {
            docker tag nginx:latest 179625949269.dkr.ecr.ap-south-1.amazonaws.com/nginx:latest
             docker push 179625949269.dkr.ecr.ap-south-1.amazonaws.com/nginx:latest
          }
        }
      }
    }
